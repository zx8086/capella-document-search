...
async function getConnection() {
  ...
  try {
    ...
    err("[ERROR] [Worker] Connection error:", error);
    throw error;
  } finally {
    isConnecting = false;
  }
...
if (parentPort) {
  parentPort.on("message", async (data: { vector: number[] }) => {
    try {
      ...
      log("[Stats] [Worker] Input vector sample:", {
      ...
      log("[Processing] [Worker] Starting Couchbase connection with details:", {
      ...
      log("[OK] [Worker] Connected to Couchbase");

      // Verify vector dimensions
      log("[Verify] [Worker] Verifying vector dimensions:", {
      ...
      const request = SearchRequest.create(
        VectorSearch.fromVectorQuery(VectorQuery.create("vectors", data.vector).numCandidates(5))
      );

      log("[Search] [Worker] Executing vector search with query:", {
        indexName: backendConfig.capella.VECTOR_INDEX,
        vectorField: "vectors",
        numCandidates: 5,
      ...
      log("[List] [Worker] Available indexes:", {
        indexCount: indexes.length,
        indexNames: indexes.map((idx) => idx.name).join(", "),
      });

      const result = await cluster.searchQuery(backendConfig.capella.VECTOR_INDEX, request, {
        timeout: 10000,
      });

      const rows = [];
      for await (const row of result.rows) {
        log("[Row] [Worker] Processing row:", {
          id: row.id,
          score: row.score,
      ...
      log(`[OK] [Worker] Search complete, found ${rows.length} results`);
      if (parentPort) {
...
function _isRetryableError(error: any): boolean {
  return (
    isConnectionError(error) || error.message.includes("timeout") || error.code === "ETIMEDOUT"
  );
}
