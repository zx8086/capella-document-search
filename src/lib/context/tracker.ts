...
const _setUserCallback: ((username: string) => void) | null = null;
...
let isStarted = false;
const graphqlTracker: any = null;
const _cachedFlags: Record<string, boolean> = {};
...
const _isSafari = browser && /^((?!chrome|android).)*safari/i.test(navigator.userAgent);
...
export function debugElasticIntegration() {
  const tracker = getTracker();
  if (!tracker) {
    console.warn("[Tracker] Tracker not available for Elastic debug");
    return;
  }

  console.group("[Tracker] Elastic APM Integration Status");
  try {
...
export function debugResourceLoading() {
  const tracker = getTracker();
  if (!tracker) {
    console.warn("[Tracker] Cannot debug resources: tracker not initialized");
    return;
  }

  console.group("[Tracker] OpenReplay Resource Loading Debug");
  try {
