...
export class AWSKnowledgeBaseRAGProvider implements RAGProvider {
    ...
    constructor() {
        ...
        // Use access keys for Knowledge Base access (not bearer token)
    ...
    async initialize() {
        ...
        this.traceablePipeline = traceable(async (message: string) => {
            log('🔄 [AWSKnowledgeBase] Processing query', { messageLength: message.length });
            ...
            const context = response.retrievalResults
                ?.map(result => ({
                    ...
                    pageNumber: this.extractPageNumber(result.location?.s3Location?.uri),
    ...
    private extractPageNumber(uri?: string): number | undefined {
        ...
        try {
            ...
            log('⚠️ [AWSKnowledgeBase] Failed to extract page number from URI', { uri });
            return 1;
        }
    }
}