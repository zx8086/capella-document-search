...
const { isThinking = false, thinkingText = "" } = $props<Props>();
  ...
  <div class="thinking-section border-l-4 border-blue-500 bg-blue-50 dark:bg-blue-900/20 dark:border-blue-400 rounded-r-md my-2">
    ...
    >
      <div class="flex items-center gap-2">
        {#if isThinking}
          <div class="h-4 w-4 animate-spin rounded-full border-2 border-blue-500 border-t-transparent flex-shrink-0"></div>
        {:else}
        ...
        <span class="font-medium text-blue-700 dark:text-blue-300">
          {isThinking ? "AI Reasoning..." : "AI Reasoning"}
        </span>
      </div>
      <svg
