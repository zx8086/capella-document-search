...
import { checkFeatureFlag, gb } from "$lib/featureFlags"; // Import the gb instance

// Make all state variables reactive using $state
let _isVisible = $state(true);
let _flags = $state<Record<string, boolean>>({});
let _isInitialized = $state(false);
let _isLoading = $state(false);
let _error = $state<string | null>(null);

async function checkAllFlags() {
  console.group("[FLAGS] Checking Feature Flags");
  _isLoading = true;
  _error = null;
...
{#if _isVisible}
    <div class="fixed top-16 right-4 z-50 w-80 p-3 border rounded-lg shadow-lg bg-white">
        ...
        <div class="mb-3 text-sm space-y-1">
            <p>Status: <span class="font-medium">{_isInitialized ? 'Initialized' : 'Not Initialized'}</span></p>
            <p>Loading: <span class="font-medium">{_isLoading ? 'Yes' : 'No'}</span></p>
            {#if _error}
                <p class="text-red-500">Error: {_error}</p>
            {/if}
        </div>
        ...
        {:else if Object.keys(_flags).length === 0}
            <div class="mt-3">
                <p class="text-gray-600 text-sm">No flags available</p>
            </div>
        {:else}
            <div class="mt-3 space-y-1">
                {#each Object.entries(_flags) as [key, value]}
