...
const privateEnv: Record<string, string> = isBrowser
  ? {}
  : (Object.fromEntries(Object.entries(process.env).filter(([_, v]) => v != null)) as Record<
      string,
      string
    >);
...
export function getPublicEnv(key?: string): string | undefined | Record<string, string> {
  if (key === undefined) {
    return publicEnv;
  }
